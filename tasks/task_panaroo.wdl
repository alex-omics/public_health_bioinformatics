...
task panaroo {
  ...
  command <<<
    ...
    # Build alignment flags conditionally
    # Note: nested WDL ~{} interpolation inside boolean substitution strings is not valid WDL 1.0
    # so we set a bash variable here instead and pass it to the panaroo call below.
    # ALIGN_FLAGS is intentionally set to empty string when align = false.
    # We use ${ALIGN_FLAGS:-} in the panaroo call rather than $ALIGN_FLAGS to explicitly
    # signal to bash (under set -u) that an empty value here is intentional, not an unset variable.
    ALIGN_FLAGS=""
    if [[ "~{align}" == "true" ]]; then
      ALIGN_FLAGS="--alignment ~{alignment_type} --aligner ~{aligner}"
    fi
